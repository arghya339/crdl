...
crInstall() {
  if su -c "id" >/dev/null 2>&1; then
    ...
    cp "$HOME/$crUNZIP/apks/ChromePublic.apk" "/sdcard/ChromePublic.apk"
    ~/adb -s $(~/adb devices | head -2 | tail -1 | cut -f1) shell pm install -r -i com.android.vending "/sdcard/ChromePublic.apk" > /dev/null 2>&1
    #~/adb -s $(~/adb devices | head -2 | tail -1 | awk '{print $1}') shell cmd package install -r -i com.android.vending "/sdcard/ChromePublic.apk" > /dev/null 2>&1
    INSTALL_STATUS=$?  # Capture exit status of the install command
    am start -n org.chromium.chrome/com.google.android.apps.chrome.Main > /dev/null 2>&1  # launch Chromium after update
    [ $? != 0 ] && ~/adb -s $(~/adb devices | head -2 | tail -1 | awk '{print $1}') shell "monkey -p org.chromium.chrome -c android.intent.category.LAUNCHER 1" > /dev/null 2>&1
